...
RUN python3 -c "f = '/opt/ros/noetic/lib/tf/view_frames'; data = open(f).read().replace('communicate()[1]', 'communicate()[1].decode(\"utf-8\")'); open(f, 'w').write(data)"
...
#COPY ./weight/best.pt "${REPO_PATH}/yolo.pt"
